...
const KPICard = ({ title, value, icon: Icon, color, bg, trend, onClick }) => (
    <motion.div whileHover={{ y: -2, scale: 1.01 }} transition={{ type: 'spring', stiffness: 300 }}
        onClick={onClick}
        className={`bg-white rounded-xl shadow-sm border border-gray-200 p-5 flex flex-col gap-3 cursor-pointer hover:border-blue-300 hover:shadow-md transition-all group`}>
        ...
        <div>
            <p className="text-2xl font-black text-gray-900 group-hover:text-blue-600 transition-colors">{value}</p>
...
/* Score tier card */
const ScoreCard = ({ emoji, label, count, color, colorMap, onClick }) => (
    <motion.div whileHover={{ y: -2 }} transition={{ type: 'spring', stiffness: 300 }}
        onClick={onClick}
        className={`p-5 rounded-xl border ${colorMap} cursor-pointer hover:shadow-md transition-all flex flex-col gap-3`}>
        <div className="flex items-center justify-between">
            <span className="text-lg font-black">{emoji} {label}</span>
            <ChevronRight size={16} className="opacity-50" />
        </div>
        <div className="flex items-baseline justify-between border-t border-black/5 pt-3">
            <span className="text-xs font-bold opacity-70 uppercase tracking-wide">Current</span>
            ...
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                <div>
                    <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
                        <Activity size={22} className="text-blue-600" /> Sales Dashboard
                    </h1>
                    <p className="text-gray-500 mt-0.5 text-sm">AI‑powered unified sales operations</p>
                </div>
            ...
            {hotAlertLeads.length > 0 && (
                <div className="bg-gradient-to-r from-red-50 to-orange-50 border border-red-200 rounded-xl p-4 mb-6">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <h2 className="text-sm font-black text-red-800 uppercase tracking-widest">Hot Lead Alert</h2>
                        ...
                        {hotAlertLeads.map((lead, i) => (
                            <motion.div key={lead.id}
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-red-100 text-red-700 font-black text-sm flex items-center justify-center shrink-0">{lead.name[0]}</div>
                                    <div>
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {metrics.priorityLeads.map((lead, idx) => (
                                <tr key={lead.id}
                                    ...
                                    <td className="py-3.5 px-5">
                                        <div className="flex items-center gap-2">
                                            <span className={`text-sm font-black ${SCORE_COLOR(lead.aiScore || 0)}`}>{lead.aiScore ?? '—'}</span>
