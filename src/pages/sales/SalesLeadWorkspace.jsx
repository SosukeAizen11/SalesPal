                        ...
                        {modal === 'call' && (
                            <div className="bg-gradient-to-b from-blue-900 to-blue-950 text-white flex flex-col">
                                ...
                                <div className="flex justify-center gap-6 pb-10">
                                    ...
                                    <button onClick={() => { addActionToLead(lead.id, 'call', 'Outbound Call', 'Manual call placed.'); setModal(null); }}
                                ...
                                <div className="p-3 bg-gray-100 border-t border-gray-200 flex items-center gap-2">
                                    <div className="flex-1 bg-white flex items-center rounded-full px-3 py-2 shadow-sm border border-gray-200">
                                        <input type="text" value={waText} onChange={e => setWaText(e.target.value)}
                                            onKeyDown={e => { if (e.key === 'Enter' && waText.trim()) { addActionToLead(lead.id, 'whatsapp', 'WhatsApp sent', waText.trim()); setModal(null); } }}
                                            placeholder="Type a message..." className="flex-1 text-sm bg-transparent outline-none" />
                                    </div>
                                    <button onClick={() => { if (waText.trim()) addActionToLead(lead.id, 'whatsapp', 'WhatsApp sent', waText.trim()); setModal(null); }}
        ...
        <div className="font-sans text-gray-900 pb-16">
            ...
            <div className="mb-5">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-2xl font-black shadow-md">
                            {lead.name[0]}
                        </div>
                        <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                    <p className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-1">AI Lead Score</p>
                    <div className="flex items-end justify-between">
                        <span className={`text-3xl font-black ${scoreColor(lead.aiScore || 0)}`}>{lead.aiScore ?? '—'}</span>
                        <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden mb-1.5">
                            <div className={`h-full rounded-full ${scoreBar(lead.aiScore || 0)}`} style={{ width: `${lead.aiScore || 0}%` }} />
                        </div>
                    </div>
                </div>
                <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                    <p className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-1">Deal Probability</p>
                    <span className="text-3xl font-black text-indigo-600">{lead.dealProbability ? `${lead.dealProbability}%` : '—'}</span>
                </div>
                <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                    <p className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-1">Intent Level</p>
                    <span className={`text-2xl font-black ${lead.scoreLabel === 'Hot' ? 'text-red-600' : lead.scoreLabel === 'Warm' ? 'text-orange-500' : 'text-sky-500'}`}>
                        {lead.scoreLabel || 'Warm'}
                    </span>
                </div>
                <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                    <p className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-1">Interactions</p>
                    <span className="text-3xl font-black text-gray-800">{(lead.timeline || []).length}</span>
                </div>
            </div>
                    ...
                    {activeTab === 'whatsapp' && (
                        <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col" style={{ minHeight: 420 }}>
                            ...
                            <div className="p-3 bg-gray-100 border-t border-gray-200 flex items-center gap-2">
                                <div className="flex-1 bg-white flex items-center rounded-full px-3 py-2 shadow-sm border border-gray-200">
                                    <input type="text" value={waText} onChange={e => setWaText(e.target.value)}
                                        onKeyDown={e => { if (e.key === 'Enter' && waText.trim()) { addActionToLead(lead.id, 'whatsapp', 'WhatsApp sent', waText.trim()); setWaText(''); } }}
                                        placeholder="Type a reply..." className="flex-1 text-sm bg-transparent outline-none" />
                                </div>
                                <button onClick={() => { if (waText.trim()) { addActionToLead(lead.id, 'whatsapp', 'WhatsApp sent', waText.trim()); setWaText(''); } }}
                                    className="w-10 h-10 bg-[#128C7E] hover:bg-[#075E54] text-white rounded-full flex items-center justify-center shadow-sm transition-colors">
                                    <Send size={15} className="ml-0.5" />
                                </button>
                            </div>
